            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="container-flex">
                        <!-- Chart Section -->
                        <div align="right">
                            <a href="{{ route('showapplyleave') }}" class="btn btn-primary py-8 fs-4 mb-4 rounded-2">Apply Leave</a>
                        </div>
                        ...
                        <div>
                            <table id="example" class="display nowrap">
                                ...
                                <tbody>
                                    @if ($leave->isEmpty())
                                        <tr>
                                            <td colspan="9" style="text-align: center; color: red;">Tiada rekod</td>
                                        </tr>
                                    @else
                                        @foreach ($leave as $item)
                                            <tr>
                                                <td style="text-align: center;">{{ $item->index }}</td>
                                                <td style="text-align: center;">{{ $item->dateapply }}</td>
                                                <td style="text-align: center;">{{ $user->name }}</td>
                                                <td style="text-align: center;">{{ $user->position }}</td>
                                                <td style="text-align: center;">{{ $item->datestart }}</td>
                                                <td style="text-align: center;">{{ $item->dateend }}</td>
                                                <td style="text-align: center;">{{ $item->daysleave }}</td>
                                                <td style="text-align: center;">{{ $item->purpose }}</td>
                                                <td style="text-align: center;">
                                                    <a href="" class="btn btn-primary">PRINT</a>
                                                    <a href="{{ route('deleteleave', $item->leaveid) }}" class="btn btn-danger" onclick="return confirm('Padam data ini?')">DELETE</a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    @endif
